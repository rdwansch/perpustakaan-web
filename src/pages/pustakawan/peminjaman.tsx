...
export default function peminjaman() {
  ...
  const [dataPeminjaman, setDataPeminjaman] = useState<Array<Peminjaman>>([]);
  ...
  const columns: TableColumn<Peminjaman>[] = [
  ...
  const handleHapusPeminjaman = async (id: number) => {
    ...
    if (!response.ok) {
      ...
      setAlertHeading(`Gagal HAPUS Buku`);
      setAlertDetail('Peminjaman tidak ditemukan ');
      return;
    }

    setAlertVariant('SUCCESS');
    setAlertHeading(`Berhasil HAPUS Buku`);
    setAlertDetail('');

    setDataPeminjaman(prev => prev.filter(p => p.id != id));
  };

  const handleEditPeminjaman = (row: Peminjaman) => {
    setShowModal(true);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setAlertVariant('DANGER');
        setAlertHeading(`Gagal EDIT Buku`);
        setAlertDetail('Kode Buku sudah ada');
        return;
      }

      setAlertVariant('SUCCESS');
      setAlertHeading(`Berhasil EDIT Buku`);
      setAlertDetail('');
  ...
  return (
    <>
      <Head>
        <title>Peminjaman | Pustakawan</title>
      </Head>
      <SideNav />
      <div className="absolute top-0 left-80 w-[80%] pt-10 pl-10">
        <h1 className="text-3xl text-gray-600">Peminjaman</h1>
        <br />
        {!isSSR ? <DataTable data={dataPeminjaman} columns={columns} pagination /> : <Loader />}
      </div>
            ...
            <div role="alert" className="container mx-auto w-11/12 max-w-full">
              <form onSubmit={handleSubmit}>
                <div className="relative rounded-lg border border-gray-400 bg-white py-8 px-5 shadow-md md:px-10">
                  ...
                  <h1 className="font-lg mb-4 font-bold leading-tight tracking-normal text-gray-800">Data Peminjaman</h1>
